<template>
	<view class="detail">
		...
		<musicHead :title="songDetail.name" :isShow="true" color="white"></musicHead>
		<view class="container" v-show="!isLoading">
			<scroll-view scroll-y="true">
				<view class="detail-play" @tap="handleIsPlay">
					<image :src="songDetail.al.picUrl" :class="{'detail-play-run' : isPlay}"></image>
				...
				<view class="detail-lyric">
					<view class="detail-lyric-wrap" :style="{transform:'translateY('+ -(lyricIndex - 1)*82 +'rpx)'}">
						<view class="detail-lyric-item"
						 :class="{active : lyricIndex == index}" v-for="(item,index) in songLyric" :key='index'>{{item.lyric}}</view>
				...
				<view class="detail-like">
					...
					<view class="detail-like-song">
							<view>{{item.name}}</view>
							<view>
							<image v-if="item.privilege.flag > 0 && item.privilege.flag < 70" src="../../static/only.png"></image>
	...
	export default {
		data() {
			return {
				...
				lyricIndex : 0,
				bgAudioManager:{},
				iconPlay:'icon-changyong_bofang',
				isPlay:false,
				isLoading:true	
			}
		},
			...
			getMusic(songId) {
				...
				Promise.all([songDetail(songId), songSimi(songId), songComment(songId), songLyric(songId), songUrl(songId)]).then(res => {
					...
					if(res[4][1].data.code == '200') {
						...
						this.iconPlay = 'icon-changyong_zanting'
						this.isPlay = false
						// #endif
						// 调用uni-app内置api播放音乐
						...
						this.bgAudioManager.onPlay(() =>{
							this.iconPlay = 'icon-changyong_bofang'
							this.isPlay = true
							this.listenLyricIndex()
						})
						// 监听音乐的暂停
...
.detail-play {
	...
	width: 580rpx;
	height: 580rpx;
	background-image: url(/static/dic.png);
